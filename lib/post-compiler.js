...
const { getPostFiles, getNewerAndOlderPosts } = reqRoot('lib/helpers/post');

function compilePosts(dir, outputDir) {
  let processes = [];
  const postFiles = getPostFiles(dir, 'md');
  postFiles.forEach(filepath => {
    processes.push(compilePost(filepath, postFiles, outputDir));
  });
...
function compilePost(filepath, postFiles, outputDir) {
